...
void SceneCA::Draw(void) {
    ...
    if (m_draw & RXD_FLOOR)
        drawFloor(lightpos, lightcol, cen[1] - 0.5 * dim[1], 30.0);

    // 境界,軸描画
    glDisable(GL_LIGHTING);
    if (m_draw & RXD_BBOX)
        drawWireCuboid(cen - 0.5f * dim, cen + 0.5f * dim, glm::vec3(0.5, 1.0, 0.5), 2.0);
    if (m_draw & RXD_AXIS)
    ...
    // スケルトンの描画
    if (m_ca && m_draw & RXD_SKELETON)
        m_ca->Draw(m_currentstep, m_scale);

    // メッシュ描画
    glScaled(m_scale, m_scale, m_scale);
    if (m_draw & RXD_MESH)
        drawPolygon(m_poly, m_draw);
...
void SceneCA::Timer(void) {
    if (m_animation_on) {
        // 描画を画像ファイルとして保存
        if (m_simg_spacing > 0 && m_currentstep % m_simg_spacing == 0)
            savedisplay(-1);
        ...
        if (m_ca) {
            ...
            // 頂点法線の更新
            CalVertexNormals(m_poly.vertices, m_poly.vertices.size(), m_poly.faces, m_poly.faces.size(),
                             m_poly.normals);
        }

        if (m_currentstep > RX_MAX_STEPS)
            m_currentstep = 0;
        m_currentstep++;
    }
}
...
void SceneCA::ImGui(GLFWwindow *window) {
    ...
    if (ImGui::Button("run a step")) {
        m_animation_on = true;
        Timer();
        m_animation_on = false;
    }
    if (ImGui::Button("reset viewpos")) {
        resetview();
    }
    ...
    if (m_ca) {
        ...
        if (ImGui::Button("arm (twist)")) {
            initCA("simple_arm_twist.bvh", "simple_arm.obj");
        }
        if (ImGui::Button("walking")) {
            initCA("simple_walking.bvh", "simple_human.obj");
        }
    }
    ImGui::Separator();
            ...
            for (int j = 0; j < n; ++j) {
                ...
                if (idx >= 0 && idx < vn) {
                    ...
                    if (!face->texcoords.empty())
                        glTexCoord2fv(glm::value_ptr(face->texcoords[j]));
