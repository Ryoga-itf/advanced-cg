...
//! 許容誤差を含めた等値判定
template <class T> inline bool RX_FEQ(const T &a, const T &b) { return (fabs(a - b) < RX_FEQ_EPS); }

//! 値のクランプ(クランプした値を返す)
template <class T> inline T RX_CLAMP(const T &x, const T &a, const T &b) { return ((x < a) ? a : (x > b) ? b : x); }

//! 1次元線型補間
template <class T> inline T RX_LERP(const T &a, const T &b, const T &t) { return a + t * (b - a); }

//! 乱数
inline double RX_RAND(const double &_min, const double &_max) {
    return (_max - _min) * (double(rand()) / (1.0 + RAND_MAX)) + _min;
}

//! スワップ
template <class T> inline void RX_SWAP(T &a, T &b) {
    T c;
    c = a;
    a = b;
    b = c;
 ...
 * @return VAOオブジェクト
 */
static inline GLuint CreateVAO(const GLfloat *vrts, GLuint nvrts, int dim = 3, const GLuint *tris = 0, GLuint ntris = 0,
                               const GLfloat *nrms = 0, GLuint nnrms = 0, const GLfloat *cols = 0, GLuint ncols = 0,
 ...
 * @param[in] vao VAOオブジェクト
 */
static inline void DeleteVAO(GLuint vao) { glDeleteVertexArrays(1, &vao); }
 ...
 */
static inline int MakeSphere(int &nvrts, vector<glm::vec3> &vrts, vector<glm::vec3> &nrms, int &ntris,
                             vector<int> &tris, float rad, int slices = 16, int stacks = 8) {
 ...
 * @return 生成したVAOオブジェクト
 */
static inline int MakeCylinder(int &nvrts, vector<glm::vec3> &vrts, vector<glm::vec3> &nrms, int &ntris,
                               vector<int> &tris, float rad, float len, int slices = 16, int stacks = 8) {
    const float pi = glm::pi<float>();
 ...
 * @return 生成したVAOオブジェクト
 */
static inline int MakeCapsule(int &nvrts, vector<glm::vec3> &vrts, vector<glm::vec3> &nrms, int &ntris,
                              vector<int> &tris, float rad, float len, int slices = 16, int stacks = 8) {
    const float pi = glm::pi<float>();
    int offset = 0;
 ...
 */
template <typename T> inline std::string RX_TO_STRING(const T &x) {
    std::stringstream ss;
    ss << x;
    return ss.str();
}
...
//! string型に<<オペレータを設定
template <typename T> inline std::string &operator<<(std::string &cb, const T &a) {
    cb += RX_TO_STRING(a);
    return cb;
}
 ...
 */
inline bool SearchCompExt(const string &fn, const string &ext) { return (fn.find(ext, 0) != string::npos); }
...
static inline void drawFloor(glm::vec3 light_pos, glm::vec3 light_color, float y = -1.0, float s = 20.0) {
    ...
    glUniform3f(glGetUniformLocation(glslFloor.Prog, "v3LightPosEye"), light_pos_eye[0], light_pos_eye[1],
                light_pos_eye[2]);
 ...
 * @param[in] color 描画色
 */
static inline void drawWireCuboid(const glm::vec3 &min, const glm::vec3 &max, const glm::vec3 &color,
 ...
 */
inline void CreateBoxPolygon(const glm::vec3 &sl0, const glm::vec3 &sl1, const int &d, vector<glm::vec3> &vrts,
    ...
    int idxs0[5][4] = {{0, 3, 2, 1}, {1, 2, 6, 5}, {5, 6, 7, 4}, {4, 7, 3, 0}, {0, 1, 5, 4}};

    int idxs1[4][4] = {{2, 3, 11, 10}, {3, 7, 15, 11}, {7, 6, 14, 15}, {6, 2, 10, 14}};
 ...
 */
static inline bool AABB_point_dist(const glm::vec3 &spos, const float &r, const int &sgn, const glm::vec3 &vMin,
                                   const glm::vec3 &vMax, float &d, glm::vec3 &n) {
    int bout = 0;
    float d0[6];
    int idx0 = -1;
 ...
 */
inline bool GetImplicitPlane(const glm::vec3 &pos, float &d, glm::vec3 &n, glm::vec3 &v, const glm::vec3 &pn,
                             const glm::vec3 &pq) {
    d = glm::dot(pq - pos, pn);
    n = pn;
    v = glm::vec3(0.0);
 ...
 * @param[out] n   最近傍点の法線方向
 */
static inline void GetImplicitOpenBox(const glm::vec3 &spos, const float &r, const int &sgn, const glm::vec3 &sl0,
                                      const glm::vec3 &sl1, float &d, glm::vec3 &n) {
    int t = 2;
    d = RX_FEQ_INF;
 ...
 * @return 交点数
 */
inline int ray_sphere(const glm::vec3 &p, const glm::vec3 &d, const glm::vec3 &sc, const float r, float &t1,
                      float &t2) {
