...
###線形トーンマッピングの処理###
hdr = cv2.imread('memorial.hdr',-1)[:,:,::-1] #HDR画像の読み込み＆BGRからRGBへの変換
hdr_Lab = cv2.cvtColor(hdr, cv2.COLOR_RGB2Lab) #RGB 色空間からLab 色空間に変換（明度(L)と色(ab)成分に分離）
hdr_L = hdr_Lab[:,:,0] #トーンマッピング対象の明度成分のみ抽出
ldr_L = 100.*hdr_L/hdr_L.max() #最大値が100になるように正規化
...
ldr_L = 10.**log_hdr_L - eps #対数domain の値を戻す
ldr_L = np.clip(ldr_L, 0.,100.) #範囲外の値をクリップ
ldr_Lab = hdr_Lab.copy() #LDR 画像の配列を用意
ldr_Lab[:,:,0] = ldr_L #トーンマッピングした明度成分を代入
