...
mask = mask/255. #0～1に正規化（コピーしたい前景領域（Ωの内側）を1,それ以外（Ωの外側）を0にする)

#行列Aを計算
I = scipy.sparse.diags(mask.ravel(), format='csc')#計算用に対角成分にマスクの値を持った画素数x画素数の疎行列を作成
A = scipy.sparse.eye(w*h)-I#Ωの外側(maskの値が0)の領域における画素と対応する対角成分に1を持つ疎行列を用意
#★Ωの内側(maskの値が1の領域)における画素と対応する行列Aの対角成分から4を減算
A += scipy.sparse.hstack((I[:, -1:], I[:, :-1]))#Ω内の画素と対応する行、かつその右隣の画素と対応する列に1を加算
#★同様に左隣に1を加算
#★同様に上隣に1を加算
#★同様に下隣に1を加算
