...
SceneEntry g_SceneEntries[] = {SceneRegister("Scene 8: SWE wave simulation", SceneSWE)};
...
void resizeCallback(GLFWwindow *window, int w, int h) { g_SceneEntries[g_SceneIndex].Resize(window, w, h); }
...
int main(int argc, char *argv[]) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (ImGui::BeginListBox("Scenes",
                                ImVec2(0, ImGui::GetTextLineHeightWithSpacing() * g_NumSceneEntries * 1.05))) {
            for (int i = 0; i < g_NumSceneEntries; i++) {
