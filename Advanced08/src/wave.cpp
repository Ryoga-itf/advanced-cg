...
void WaveSWE::bnd(float *u, float *v) {
    for (int i = 0; i < m_nx; ++i) {
        u[IDX(i, 0)] = u[IDX(i, 1)];
        u[IDX(i, m_ny - 1)] = u[IDX(i, m_ny - 2)]; // 壁に平行な成分は単純にコピー
        // v[IDX(i, 0)] = v[IDX(i, m_ny-1)] = 0.0f;
        v[IDX(i, 0)] = v[IDX(i, 1)];
        v[IDX(i, m_ny - 1)] = v[IDX(i, m_ny - 2)]; // 手前と奥の面は波が抜けるようにする
    }
    for (int j = 0; j < m_ny; ++j) {
        u[IDX(0, j)] = u[IDX(m_nx - 1, j)] = 0.0f; // 左右の面は壁面
        v[IDX(0, j)] = v[IDX(1, j)];
        v[IDX(m_nx - 1, j)] = v[IDX(m_nx - 2, j)]; // 壁に平行な成分は単純にコピー
    }
}
    ...
    for (int k = 0; k < 10; ++k) { // ガウス・ザイデル反復
        for (int j = 1; j < m_ny - 1; ++j) {
            for (int i = 1; i < m_nx - 1; ++i) {
                ...
                u_new[idx] =
                    u_new[idx] + dt * m_nu *
                                     ((u_new[IDX(i + 1, j)] - 2 * u_new[idx] + u_new[IDX(i - 1, j)]) / (dx * dx) +
                                      (u_new[IDX(i, j + 1)] - 2 * u_new[idx] + u_new[IDX(i, j - 1)]) / (dy * dy));
                v_new[idx] =
                    v_new[idx] + dt * m_nu *
                                     ((v_new[IDX(i + 1, j)] - 2 * v_new[idx] + v_new[IDX(i - 1, j)]) / (dx * dx) +
                                      (v_new[IDX(i, j + 1)] - 2 * v_new[idx] + v_new[IDX(i, j - 1)]) / (dy * dy));
            }
        }
        bnd(u_new, v_new);
    }
