...
class WaveSWE {
    ...
    vector<float> m_u, m_v;         //!< 速度場(u,v) (staggered: u=(nx+1)*ny, v=nx*(ny+1))
  ...
  public:
    ...
    // Staggered grid indices (Assignment B)
    // u: (m_nx+1) x m_ny, defined on x-faces at (i-0.5)*dx, j*dy
    // v: m_nx x (m_ny+1), defined on y-faces at i*dx, (j-0.5)*dy
  ...
  public:
    ...
    void GetDim(glm::vec3 &cen, glm::vec3 &dim) {
        cen = 0.5f * (m_min + m_max);
        dim = (m_max - m_min);
    }
