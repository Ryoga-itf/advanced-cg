...
bool TriMesh::loadObj(const char *filename) {
    ...
    for (unsigned int gi = 0; gi < m->group_count; ++gi) {
        ...
        for (unsigned int groupFaceIndex = 0; groupFaceIndex < grp.face_count; ++groupFaceIndex) {
            ...
            for (int vi = 2; vi < nFaceVertices; ++vi) // defining triangles
            {
                ...
                if (m0.t) {
                    if (m0.n)
                        m_TriangleIndices.emplace_back(VertexTuple(m0.p - 1, m0.t - 1, m0.n - 1),
                                                       VertexTuple(m1.p - 1, m1.t - 1, m1.n - 1),
                                                       VertexTuple(m2.p - 1, m2.t - 1, m2.n - 1));
                    else
                        m_TriangleIndices.emplace_back(VertexTuple(m0.p - 1, m0.t - 1, -1),
                                                       VertexTuple(m1.p - 1, m1.t - 1, -1),
                                                       VertexTuple(m2.p - 1, m2.t - 1, -1));
                } else {
                    if (m0.n)
                        m_TriangleIndices.emplace_back(VertexTuple(m0.p - 1, -1, m0.n - 1),
                                                       VertexTuple(m1.p - 1, -1, m1.n - 1),
                                                       VertexTuple(m2.p - 1, -1, m2.n - 1));
                    else
                        m_TriangleIndices.emplace_back(VertexTuple(m0.p - 1), VertexTuple(m1.p - 1),
                                                       VertexTuple(m2.p - 1));
                }
    ...
    if (nNonTriangles)
        cerr << __FUNCTION__ << ": Note: " << nNonTriangles
             << " non-triangles found (automatically converted to triangles)" << endl;
...
void TriMesh::calcVertexNormals() {
    ...
    // #pragma omp parallel
    {
        ...
        for (int ni = 0; ni < nVerts; ni++)
            m_VertexNormals[ni] = glm::vec3(0.0);
        ...
        for (int fi = 0; fi < nFaces; fi++) {
            ...
            if (i0 >= nVerts || i1 >= nVerts || i2 >= nVerts || i0 < 0 || i1 < 0 || i2 < 0) {
                cerr << __FUNCTION__ << ": warning: triangle index out of range: (" << i0 << "," << i1 << "," << i2
                     << ")" << endl;
            }

            const glm::vec3 weightedNormal =
                glm::cross((m_Vertices[i1] - m_Vertices[i0]), (m_Vertices[i2] - m_Vertices[i1]));
        ...
        // #pragma omp for
        for (int ni = 0; ni < nVerts; ni++)
            m_VertexNormals[ni] = glm::normalize(m_VertexNormals[ni]);
    }
}

void TriMesh::renderTexturedMesh() const {
    if (!m_TexID || !m_VertexVBO || !m_TexCoordVBO) {
        cerr << __FUNCTION__ << ": texID = " << m_TexID << ", vertex VBO = " << m_VertexVBO
             << ", tex coord VBO = " << m_TexCoordVBO << endl;
        return;
    }
...
void TriMesh::bakeVBOs() {
    ...
    if (nNormals)
        tmpNormals.resize(3 * nTriangles);
    if (nTexCoords)
        tmpTexCoords.resize(3 * nTriangles);
