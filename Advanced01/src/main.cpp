...
#include "Scene01Checker2D.hpp"
#include "Scene02ImageSmoothing.hpp"
#include "Scene03WaveAnimation.hpp"
#include "Scene04PseudoNormal.hpp"
...
#define SceneRegister(name, className)                                                                                 \
    {name,                                                                                                             \
     (className::Init),                                                                                                \
     (className::ReloadShaders),                                                                                       \
     (className::Draw),                                                                                                \
     (className::Cursor),                                                                                              \
     (className::Mouse),                                                                                               \
     (className::Resize),                                                                                              \
...
SceneEntry g_SceneEntries[] = {SceneRegister("Scene 01: Checker 2D", Scene01Checker2D),
                               ...
                               SceneRegister("Scene 06: Life", Scene06)};
...
void errorCallback(int error, const char *description) { cout << __FUNCTION__ << ": " << description << endl; }
...
int main(int argc, char **argv) {
    ...
    while (!glfwWindowShouldClose(pWindow)) {
        ...
        // gui
        {
            ...
            if (ImGui::BeginListBox("Scenes", ImVec2(0, ImGui::GetTextLineHeightWithSpacing() * g_NumSceneEntries *
                                                            1.05))) // 1.05 for padding
            {
                for (int i = 0; i < g_NumSceneEntries; i++) {
