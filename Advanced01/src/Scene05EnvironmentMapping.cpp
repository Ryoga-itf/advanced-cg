#include "Scene05EnvironmentMapping.hpp"
...
void Scene05EnvironmentMapping::Init() {
    ...
    {
        int w, h;
        Image2OGLTexture(finder.find("venice_sunrise_2k.jpg").c_str(), s_EnvMapTexID, w, h, GL_TEXTURE_2D, GL_LINEAR,
                         GL_MIRRORED_REPEAT);
        // Image2OGLTexture(finder.find("kasuga_envmap.jpg").c_str(), s_EnvMapTexID, w, h, GL_TEXTURE_2D, GL_LINEAR,
        // GL_MIRRORED_REPEAT);
    }
...
void Scene05EnvironmentMapping::Resize(GLFWwindow *window, int w, int h) { AbstractScene::Resize(window, w, h); }
...
void Scene05EnvironmentMapping::BuildSkyDome() {
    ...
    for (int vi = 0; vi < nDivSphereLatitude; vi++) {
        ...
        for (int ui = 0; ui < nDivSphereLongitude; ui++) {
            ...
            vertices.emplace_back(radius * sinf(lat0) * cosf(lon1), radius * cosf(lat0),
                                  radius * sinf(lat0) * sinf(lon1));
            vertices.emplace_back(radius * sinf(lat0) * cosf(lon0), radius * cosf(lat0),
                                  radius * sinf(lat0) * sinf(lon0));
            vertices.emplace_back(radius * sinf(lat1) * cosf(lon0), radius * cosf(lat1),
                                  radius * sinf(lat1) * sinf(lon0));
            vertices.emplace_back(radius * sinf(lat0) * cosf(lon1), radius * cosf(lat0),
                                  radius * sinf(lat0) * sinf(lon1));
            vertices.emplace_back(radius * sinf(lat1) * cosf(lon0), radius * cosf(lat1),
                                  radius * sinf(lat1) * sinf(lon0));
            vertices.emplace_back(radius * sinf(lat1) * cosf(lon1), radius * cosf(lat1),
