...
void Scene06::Resize(GLFWwindow *window, int w, int h) { AbstractScene::Resize(window, w, h); }
...
void Scene06::CreateStateResources(int w, int h) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, s_StateTex[i], 0);
    }
