...
struct VertexTuple {
    ...
    VertexTuple(int _vertexIdx, int _textureIdx, int _normalIdx)
        : vertexIdx(_vertexIdx), textureIdx(_textureIdx), normalIdx(_normalIdx) {}

    void set(int _vertexIdx) {
        vertexIdx = _vertexIdx;
        textureIdx = _vertexIdx;
        normalIdx = _vertexIdx;
    }
    void set(int _vertexIdx, int _textureIdx, int _normalIdx) {
        vertexIdx = _vertexIdx;
        textureIdx = _textureIdx;
        normalIdx = _normalIdx;
    }
...
struct TriangleIndices {
    ...
    TriangleIndices(const VertexTuple &_v0, const VertexTuple &_v1, const VertexTuple &_v2)
        : indices({_v0, _v1, _v2}) {}
